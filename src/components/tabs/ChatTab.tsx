...
import { Mic, Play, Pause, Square, User } from "lucide-react";
import { motion, AnimatePresence } from "framer-motion";
import { formatMessage } from "../../utils/messageFormatter";
import LoaderRing from "../ui/Loader";
...
interface ChatTabProps {
  /** Called when the header profile button is clicked */
  goToProfile?: () => void;
}

const ChatTab: React.FC<ChatTabProps> = ({ goToProfile }) => {
  const [input, setInput] = useState("");
  const [messages, setMessages] = useState<any[]>([]);
  const [sessionId, setSessionId] = useState<string | null>(null);
  const [isTyping, setIsTyping] = useState(false);
  ...
  // Smooth auto-scroll; only if user is near bottom unless forced
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // 1) { data: { profile: {...} } }
        // 2) { data: { data: { profile: {...} } } }
        const root = res?.data as any; // relax typing only at the boundary
        const profile =
          root?.data?.profile ?? // shape 1
          root?.data?.data?.profile; // shape 2
  ...
  // Scroll AFTER layout/paint and Framer Motion has had a chance to animate
  useEffect(() => {
    let raf1 = 0;
    let raf2 = 0;
  ...
  }, [messages, isTyping, isProcessingVoice]);

  // Keep pinned to bottom when the container size changes (e.g., rich content, images)
  useEffect(() => {
  ...
  // const handleMicClick = () => { ...speech recognition code... }
  // const handleCardClick = async (recommendation: any) => { ... }
    ...
    <div className="h-screen flex flex-col bg-black">
      ...
      <div className="px-6 py-3">
        <div className="flex items-center justify-between">
          ...
          {/* Right: Profile button (same logic as old: go to profile tab) */}
          <button
            onClick={() => (goToProfile ? goToProfile() : null)}
          ...
          <AnimatePresence initial={false}>
            {messages.length === 0 ? (
              ...
              <div className="mt-4 space-y-4">
                ...
                {/* Agent Processing Indicator — only during API calls */}
                {(isTyping || isProcessingVoice) && (
                  <motion.div
                    initial={{ opacity: 0, y: 10 }}
                    animate={{ opacity: 1, y: 0 }}
                  ...
                  >
                    <div className="flex flex-col items-center">
                      <LoaderRing size={72} stroke={6} label="Loading..." />
          ...
          >
            <div className="relative">
              ...
              <input
                ...
                className="w-full h-14 rounded-full bg-white text-slate-900 placeholder-slate-500
                    pl-5 pr-20 shadow-[0_2px_14px_rgba(0,0,0,0.25)] focus:outline-none
            ...
            {/* optional: keep a visually-hidden submit so Enter still sends */}
